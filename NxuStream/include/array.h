...
// elements to an pxArray is still O(n).
// The memory usage is also of the same order as if a C array was used.
// An pxArray uses less than double the minimum needed space.  Again, we 
...
#ifndef PHYSX_ARRAY_H
#define PHYSX_ARRAY_H

#include <assert.h>
#include <stdio.h>

template <class Type> class pxArrayRet;  
template <class Type> class pxArray {
  public:
				pxArray(int s=0);
				pxArray(pxArray<Type> &array);
				pxArray(pxArrayRet<Type> &array);
				~pxArray();
	void		allocate(int s);
	void		SetSize(int s);
	void		Pack();
	...
	pxArray<Type> &operator=(pxArray<Type> &array);
	pxArray<Type> &operator=(pxArrayRet<Type> &array);
	// operator pxArrayRet<Type> &() { return *(pxArrayRet<Type> *)this;} // this worked but i suspect could be dangerous
};

template <class Type> class pxArrayRet:public pxArray<Type>
{
};

template <class Type> pxArray<Type>::pxArray(int s)
{
	count=0;
	array_size = 0;
	element = NULL;
...
template <class Type> pxArray<Type>::pxArray(pxArray<Type> &array)
{
	count=0;
	array_size = 0;
	element = NULL;
...
template <class Type> pxArray<Type>::pxArray(pxArrayRet<Type> &array)
{
	*this = array;
}
template <class Type> pxArray<Type> &pxArray<Type>::operator=(pxArrayRet<Type> &array)
{
	count=array.count;
...
template <class Type> pxArray<Type> &pxArray<Type>::operator=(pxArray<Type> &array)
{
	count=0;
	for(int i=0;i<array.count;i++) 
    {
...
template <class Type> pxArray<Type>::~pxArray()
{
    if (element != NULL)
    {
        delete[] element;
...
template <class Type> void pxArray<Type>::allocate(int s)
{
	assert(s>0);
	assert(s>=count);
	Type *old = element;
...
template <class Type> void pxArray<Type>::SetSize(int s)
{
	if(s==0) 
    { 
        if(element) 
...
template <class Type> void pxArray<Type>::Pack()
{
	allocate(count);
}

template <class Type> Type& pxArray<Type>::Add(Type t)
{
...
template <class Type> int pxArray<Type>::Contains(Type t)
{
	int i;
	int found=0;
	for(i=0;i<count;i++) 
...
template <class Type> void pxArray<Type>::AddUnique(Type t)
{
	if(!Contains(t)) Add(t);
}


template <class Type> void pxArray<Type>::DelIndex(int i)
{
	assert(i<count);
	count--;
	while(i<count)
...
template <class Type> void pxArray<Type>::Remove(Type t)
{
	int i;
	for(i=0;i<count;i++) 
    {
...
template <class Type> void pxArray<Type>::Insert(Type t,int k)
{
	int i=count;
...
template <class Type> int pxArray<Type>::IndexOf(Type t)
{
	int i;
	for(i=0;i<count;i++) 
    {
