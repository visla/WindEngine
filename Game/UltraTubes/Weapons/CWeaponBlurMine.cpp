...
void CWeaponBlurMine::InitializeWeapon(CTuber *Owner, WeVector &RelativeSpawnLoc, WeVector &WeapLocation, WeVector &WeapDirection, WeMatrix &WeapOrientation)
{
	Type = ACTORTYPE_MINE;
	LifeTime = LIFETIME;
	...
	Model = WeModelManager::Instance()->Load("Weapons.Models.BlurMine");
	...
	NotifyTargetArea->PhysicHandle = WePhysics::Instance()->CreateSphere(NotifyTargetArea, Location, GetEffectiveRadius(WEAPONTYPE_BLURMINE), COLLISION_GROUP_NOTIFY_TRIGGER, 10.0f, false, WETRIGGER_ON_ENABLE);
	...
	Sound = WeAudioDevice::Instance()->LoadSoundAuto("Sounds.Mines.BlurMine", true);
	if (Sound)
	{
...
CWeaponBlurMine::CWeaponBlurMine(CTuber* Owner, WeVector &RelativeSpawnLoc, WeMatrix &WeapOwnerMatrix)
{
...
CWeaponBlurMine::~CWeaponBlurMine()
{
	NotifyTargetArea->LifeTime = 0.0001f;
	SAFE_DELETE(Sound);
}

void CWeaponBlurMine::Tick()
{
...
void CWeaponBlurMine::OnHit(WeActor *Victim)
{
    if (Dying) return;
	...
	if ( (Victim->Type == ACTORTYPE_WEAPON) || (Victim->Type == ACTORTYPE_EXPLOSION) )
	{
		...
		Explosion = new CExplosionLocalized(Location, WeaponOwner, WeaponDesc.Damage, NULL, LocalizedExplosionBlurMine, WEAPONTYPE_BLURMINE);
	}
}

void CWeaponBlurMine::OnNotify(WeActor *Victim)
{
	if (Dying) return;
	if (ActivationTime < GetTimeToActivate(WEAPONTYPE_BLURMINE)) return;

	CTuber *PlayerVictim = NULL;
	if (ACTORTYPE_TUBER == Victim->Type)
		PlayerVictim = (CTuber*)Victim;

	if (PlayerVictim != CMainGamePhase::Instance->InputPlayer) return;

	float Distance = D3DXVec3Length(&(Victim->Location - Location));
	float NewFactor = ((Distance / GetEffectiveRadius(WEAPONTYPE_BLURMINE)) * 0.75f);
	PlayerVictim->AddonBlurFactor = (PlayerVictim->AddonBlurFactor < NewFactor) ? NewFactor : PlayerVictim->AddonBlurFactor;
}
