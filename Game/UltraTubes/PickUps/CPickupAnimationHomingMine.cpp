/*
       File: CPickupAnimationHomingMine.cpp
...
CPickupAnimationHomingMine::CPickupAnimationHomingMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_HOMINGMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("HomingMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_HOMINGMINE)->PickUpAmmo);
	UberText = String(uber);
	UberTextColor = COLOR_SECONDARY;
	OutputUberText();

	WeSound *PickupAudio = WeAudioDevice::Instance()->LoadSoundAuto("Sounds.Voice.PickupHomingMine", false);
...
void CPickupAnimationHomingMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationHomingMine::Render()
{
	if(Mine)
	{
