/*
       File: CPickupAnimationStandardMine.cpp
...
CPickupAnimationStandardMine::CPickupAnimationStandardMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_STANDARDMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("StandardMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_STANDARDMINE)->PickUpAmmo);
...
void CPickupAnimationStandardMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationStandardMine::Render()
{
	if(Mine)
	{
