/*
       File: CPickupAnimationSuperNukeMine.cpp
...
CPickupAnimationSuperNukeMine::CPickupAnimationSuperNukeMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_SUPERNUKEMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("SuperNukeMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_SUPERNUKEMINE)->PickUpAmmo);
...
void CPickupAnimationSuperNukeMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationSuperNukeMine::Render()
{
	if(Mine)
	{
