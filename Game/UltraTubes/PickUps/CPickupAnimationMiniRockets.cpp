/*
       File: CPickupAnimationMiniRockets.cpp
...
CPickupAnimationMiniRockets::CPickupAnimationMiniRockets()
{
	Weapon = GetAnimTexture(WEAPONTYPE_MINIROCKETS);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("MiniRockets").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_MINIROCKETS)->PickUpAmmo);
...
void CPickupAnimationMiniRockets::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationMiniRockets::Render()
{
	if(Weapon)
	{
