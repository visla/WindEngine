/*
       File: CPickupAnimationSlowingRocket.cpp
...
CPickupAnimationSlowingRocket::CPickupAnimationSlowingRocket()
{
	Weapon = GetAnimTexture(WEAPONTYPE_SLOWINGROCKET);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("SlowingRocket").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_SLOWINGROCKET)->PickUpAmmo);
	UberText = String(uber);
	UberTextColor = COLOR_PRIMARY;
	OutputUberText();

	WeSound *PickupAudio = WeAudioDevice::Instance()->LoadSoundAuto("Sounds.Voice.PickupSlowingRocket", false);
...
void CPickupAnimationSlowingRocket::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationSlowingRocket::Render()
{
	if(Weapon)
	{
