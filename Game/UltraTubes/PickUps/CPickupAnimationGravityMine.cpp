/*
       File: CPickupAnimationGravityMine.cpp
...
CPickupAnimationGravityMine::CPickupAnimationGravityMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_GRAVITYMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("GravityMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_GRAVITYMINE)->PickUpAmmo);
...
void CPickupAnimationGravityMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationGravityMine::Render()
{
	if(Mine)
	{
