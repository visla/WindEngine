/*
File: CPickupAnimationStandardLaser.cpp
...
CPickupAnimationStandardLaser::CPickupAnimationStandardLaser()
{
	Weapon = GetAnimTexture(WEAPONTYPE_STANDARDLASER);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("StandardLaser").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_STANDARDLASER)->PickUpAmmo);
	UberText = String(uber);
	UberTextColor = COLOR_PRIMARY;
	OutputUberText();

	WeSound *PickupAudio = WeAudioDevice::Instance()->LoadSoundAuto("Sounds.Voice.PickupStandardBolt", false);
...
void CPickupAnimationStandardLaser::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationStandardLaser::Render()
{
	if(Weapon)
	{
