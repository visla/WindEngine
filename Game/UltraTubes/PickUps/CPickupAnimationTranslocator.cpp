/*
       File: CPickupAnimationTranslocator.cpp
...
CPickupAnimationTranslocator::CPickupAnimationTranslocator()
{
	Weapon = GetAnimTexture(WEAPONTYPE_TRANSLOCATOR);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("Translocator").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_TRANSLOCATOR)->PickUpAmmo);
	UberText = String(uber);
	UberTextColor = COLOR_PRIMARY;
	OutputUberText();

	WeSound *PickupAudio = WeAudioDevice::Instance()->LoadSoundAuto("Sounds.Voice.PickupTranslocator", false);
...
void CPickupAnimationTranslocator::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationTranslocator::Render()
{
	if(Weapon)
	{
