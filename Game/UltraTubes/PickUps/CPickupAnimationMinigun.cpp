/*
       File: CPickupAnimationMinigun.cpp
...
CPickupAnimationMinigun::CPickupAnimationMinigun()
{
	Weapon = GetAnimTexture(WEAPONTYPE_MINIGUN);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("Minigun").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_MINIGUN)->PickUpAmmo);
...
void CPickupAnimationMinigun::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationMinigun::Render()
{
	if(Weapon)
	{
