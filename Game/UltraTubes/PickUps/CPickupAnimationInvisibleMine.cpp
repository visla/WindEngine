/*
       File: CPickupAnimationInvisibleMine.cpp
...
CPickupAnimationInvisibleMine::CPickupAnimationInvisibleMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_INVISIBLEMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("InvisibleMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_INVISIBLEMINE)->PickUpAmmo);
...
void CPickupAnimationInvisibleMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationInvisibleMine::Render()
{
	if(Mine)
	{
