/*
       File: CPickupAnimationQuantumLaser.cpp
...
CPickupAnimationQuantumLaser::CPickupAnimationQuantumLaser()
{
	Weapon = GetAnimTexture(WEAPONTYPE_QUANTUMLASER);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("QuantumLaser").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_QUANTUMLASER)->PickUpAmmo);
	UberText = String(uber);
	UberTextColor = COLOR_PRIMARY;
	OutputUberText();

	WeSound *PickupAudio = WeAudioDevice::Instance()->LoadSoundAuto("Sounds.Voice.PickupQuantumBolt", false);
...
void CPickupAnimationQuantumLaser::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationQuantumLaser::Render()
{
	if(Weapon)
	{
