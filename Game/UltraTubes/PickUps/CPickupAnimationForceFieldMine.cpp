/*
       File: CPickupAnimationForceFieldMine.cpp
...
CPickupAnimationForceFieldMine::CPickupAnimationForceFieldMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_FORCEFIELDMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("ForcefieldMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_FORCEFIELDMINE)->PickUpAmmo);
...
void CPickupAnimationForceFieldMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationForceFieldMine::Render()
{
	if(Mine)
	{
