/*
File: CPickupAnimationQuadLaser.cpp
...
CPickupAnimationQuadLaser::CPickupAnimationQuadLaser()
{
	Weapon = GetAnimTexture(WEAPONTYPE_QUADLASER);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("QuadLaser").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_QUADLASER)->PickUpAmmo);
...
void CPickupAnimationQuadLaser::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationQuadLaser::Render()
{
	if(Weapon)
	{
