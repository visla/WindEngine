/*
       File: CPickupAnimationNukeRocket.cpp
...
CPickupAnimationNukeRocket::CPickupAnimationNukeRocket()
{
	Weapon = GetAnimTexture(WEAPONTYPE_NUKEROCKET);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("NukeRocket").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_NUKEROCKET)->PickUpAmmo);
...
void CPickupAnimationNukeRocket::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationNukeRocket::Render()
{
	if(Weapon)
	{
