/*
File: CPickupAnimationDoubleLaser.cpp
...
CPickupAnimationDoubleLaser::CPickupAnimationDoubleLaser()
{
	Weapon = GetAnimTexture(WEAPONTYPE_DOUBLELASER);
	Mine = NULL;
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("DoubleLaser").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_DOUBLELASER)->PickUpAmmo);
...
void CPickupAnimationDoubleLaser::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationDoubleLaser::Render()
{
	if(Weapon)
	{
