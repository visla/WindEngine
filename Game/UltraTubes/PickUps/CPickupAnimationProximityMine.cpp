/*
       File: CPickupAnimationProximityMine.cpp
...
CPickupAnimationProximityMine::CPickupAnimationProximityMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_PROXIMITYMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("ProximityMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_PROXIMITYMINE)->PickUpAmmo);
...
void CPickupAnimationProximityMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationProximityMine::Render()
{
	if(Mine)
	{
