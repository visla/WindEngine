/*
       File: CPickupAnimationSlowingMine.cpp
...
CPickupAnimationSlowingMine::CPickupAnimationSlowingMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_SLOWINGMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("SlowingMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_SLOWINGMINE)->PickUpAmmo);
...
void CPickupAnimationSlowingMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationSlowingMine::Render()
{
	if(Mine)
	{
