/*
       File: CPickupAnimationBlurMine.cpp
...
CPickupAnimationBlurMine::CPickupAnimationBlurMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_BLURMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("BlurMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_BLURMINE)->PickUpAmmo);
...
void CPickupAnimationBlurMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationBlurMine::Render()
{
	if(Mine)
	{
