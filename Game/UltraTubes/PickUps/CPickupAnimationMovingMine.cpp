/*
       File: CPickupAnimationMovingMine.cpp
...
CPickupAnimationMovingMine::CPickupAnimationMovingMine()
{
	Mine = GetAnimTexture(WEAPONTYPE_MOVINGMINE);
	...
	char uber[100];
	sprintf(uber, "%s (%d)", CLocalizationSettings::Instance()->GetLocalText("MovingMine").GetString(), CWeaponDescriptionManager::Instance->GetWeaponDescription(WEAPONTYPE_MOVINGMINE)->PickUpAmmo);
...
void CPickupAnimationMovingMine::Tick()
{
	if(FirstTime)
	{
...
void CPickupAnimationMovingMine::Render()
{
	if(Mine)
	{
